    ...
    appendTransaction: (transactions, oldState, newState) => {
      ...
      changes.forEach(({ oldRange, newRange }) => {
        ...
        findChildrenInRange(newState.doc, newRange, (node) => node.isTextblock).forEach((textBlock) => {
          ...
          find(text2).filter((link) => link.isLink).map((link) => ({
            ...link,
...
var Link = Mark.create({
  ...
  addPasteRules() {
    return [
      markPasteRule({
        find: (text2) => find(text2).filter((link) => link.isLink).map((link) => ({
          text: link.value,
          index: link.start,
          data: link
        })),
...
// dep:@tiptap_extension-link
var tiptap_extension_link_default = Link;
export {
  Link,
  tiptap_extension_link_default as default
};
