...
OrderedMap.prototype = {
  ...
  prepend: function(map13) {
    map13 = OrderedMap.from(map13);
    if (!map13.size)
      return this;
    return new OrderedMap(map13.content.concat(this.subtract(map13).content));
  },
  append: function(map13) {
    map13 = OrderedMap.from(map13);
    if (!map13.size)
      return this;
    return new OrderedMap(this.subtract(map13).content.concat(map13.content));
  },
  subtract: function(map13) {
    var result2 = this;
    map13 = OrderedMap.from(map13);
    for (var i = 0; i < map13.content.length; i += 2)
      result2 = result2.remove(map13.content[i]);
    return result2;
  },
  get size() {
...
Fragment.prototype.nodesBetween = function nodesBetween(from3, to, f, nodeStart, parent) {
  if (nodeStart === void 0)
    nodeStart = 0;
  for (var i = 0, pos = 0; pos < to; i++) {
    var child3 = this.content[i], end2 = pos + child3.nodeSize;
    if (end2 > from3 && f(child3, nodeStart + pos, parent, i) !== false && child3.content.size) {
      var start3 = pos + 1;
      child3.nodesBetween(
        Math.max(0, from3 - start3),
...
Fragment.prototype.textBetween = function textBetween(from3, to, blockSeparator, leafText) {
  var text2 = "", separated = true;
  this.nodesBetween(from3, to, function(node4, pos) {
    if (node4.isText) {
      text2 += node4.text.slice(Math.max(from3, pos) - pos, to - pos);
...
Fragment.prototype.cut = function cut(from3, to) {
  if (to == null) {
    to = this.size;
  }
    ...
    for (var i = 0, pos = 0; pos < to; i++) {
      ...
      if (end2 > from3) {
        if (pos < from3 || end2 > to) {
          if (child3.isText) {
            child3 = child3.cut(Math.max(0, from3 - pos), Math.min(child3.text.length, to - pos));
          } else {
            child3 = child3.cut(Math.max(0, from3 - pos - 1), Math.min(child3.content.size, to - pos - 1));
          }
        }
        result2.push(child3);
...
Fragment.prototype.cutByIndex = function cutByIndex(from3, to) {
  if (from3 == to) {
...
Slice.prototype.removeBetween = function removeBetween(from3, to) {
...
function removeRange(content2, from3, to) {
  var ref = content2.findIndex(from3);
  ...
  if (offset2 == from3 || child3.isText) {
    if (offsetTo != to && !content2.child(indexTo).isText) {
      throw new RangeError("Removing non-flat range");
    }
    return content2.cut(0, from3).append(content2.cut(to));
  }
  if (index2 != indexTo) {
    throw new RangeError("Removing non-flat range");
  }
  return content2.replaceChild(index2, child3.copy(removeRange(child3.content, from3 - offset2 - 1, to - offset2 - 1)));
}
...
function replaceOuter($from, $to, slice3, depth) {
  ...
  if (index2 == $to.index(depth) && depth < $from.depth - slice3.openStart) {
    var inner = replaceOuter($from, $to, slice3, depth + 1);
    ...
    return close(parent, content2.cut(0, $from.parentOffset).append(slice3.content).append(content2.cut($to.parentOffset)));
  } else {
    var ref = prepareSliceForReplace(slice3, $from);
...
function prepareSliceForReplace(slice3, $along) {
  var extra = $along.depth - slice3.openStart, parent = $along.node(extra);
  var node4 = parent.copy(slice3.content);
...
Node.prototype.nodesBetween = function nodesBetween2(from3, to, f, startPos) {
  if (startPos === void 0)
    startPos = 0;
...
Node.prototype.textBetween = function textBetween2(from3, to, blockSeparator, leafText) {
...
Node.prototype.cut = function cut2(from3, to) {
  if (from3 == 0 && to == this.content.size) {
    return this;
  }
  return this.copy(this.content.cut(from3, to));
};
Node.prototype.slice = function slice(from3, to, includeParents) {
  if (to === void 0)
  ...
  var $from = this.resolve(from3), $to = this.resolve(to);
...
Node.prototype.replace = function replace$1(from3, to, slice3) {
...
Node.prototype.rangeHasMark = function rangeHasMark(from3, to, type) {
  var found2 = false;
  if (to > from3) {
    this.nodesBetween(from3, to, function(node4) {
...
Node.prototype.canReplace = function canReplace(from3, to, replacement, start3, end2) {
  ...
  var one = this.contentMatchAt(from3).matchFragment(replacement, start3, end2);
...
Node.prototype.canReplaceWith = function canReplaceWith(from3, to, type, marks2) {
  if (marks2 && !this.type.allowsMarks(marks2)) {
    return false;
  }
  var start3 = this.contentMatchAt(from3).matchType(type);
...
var TextNode = function(Node4) {
  ...
  TextNode2.prototype.textBetween = function textBetween3(from3, to) {
  ...
  TextNode2.prototype.cut = function cut3(from3, to) {
    if (from3 === void 0)
      from3 = 0;
    if (to === void 0)
      to = this.text.length;
...
function nfa(expr) {
  ...
  function edge2(from3, to, term) {
    var edge3 = { term, to };
    nfa2[from3].push(edge3);
    return edge3;
  }
  ...
  function compile3(expr2, from3) {
    if (expr2.type == "choice") {
      return expr2.exprs.reduce(function(out, expr3) {
        return out.concat(compile3(expr3, from3));
      }, []);
    } else if (expr2.type == "seq") {
      for (var i = 0; ; i++) {
        var next = compile3(expr2.exprs[i], from3);
        if (i == expr2.exprs.length - 1) {
          return next;
        }
        connect(next, from3 = node4());
      }
      ...
      var loop$1 = node4();
      connect(compile3(expr2.expr, from3), loop$1);
      connect(compile3(expr2.expr, loop$1), loop$1);
      return [edge2(loop$1)];
    } else if (expr2.type == "opt") {
      return [edge2(from3)].concat(compile3(expr2.expr, from3));
      ...
      return [edge2(from3, null, expr2.value)];
    }
  }
}
function cmp(a, b) {
...
var Mapping = function Mapping2(maps, mirror, from3, to) {
  this.maps = maps || [];
  this.from = from3 || 0;
  this.to = to == null ? this.maps.length : to;
  this.mirror = mirror;
};
Mapping.prototype.slice = function slice2(from3, to) {
  if (from3 === void 0)
    from3 = 0;
  if (to === void 0)
    to = this.maps.length;
...
Mapping.prototype.appendMap = function appendMap(map13, mirrors) {
  this.to = this.maps.push(map13);
  if (mirrors != null) {
...
Mapping.prototype._map = function _map2(pos, assoc, simple) {
  var deleted = false;
  for (var i = this.from; i < this.to; i++) {
    var map13 = this.maps[i], result2 = map13.mapResult(pos, assoc);
...
StepResult.fromReplace = function fromReplace(doc2, from3, to, slice3) {
  try {
...
var ReplaceStep = function(Step3) {
  function ReplaceStep2(from3, to, slice3, structure) {
    Step3.call(this);
    this.from = from3;
    this.to = to;
    this.slice = slice3;
  ...
  ReplaceStep2.prototype.map = function map13(mapping) {
    var from3 = mapping.mapResult(this.from, 1), to = mapping.mapResult(this.to, -1);
    if (from3.deleted && to.deleted) {
      return null;
    }
    return new ReplaceStep2(from3.pos, Math.max(from3.pos, to.pos), this.slice);
  };
...
var ReplaceAroundStep = function(Step3) {
  function ReplaceAroundStep2(from3, to, gapFrom, gapTo, slice3, insert, structure) {
    Step3.call(this);
    this.from = from3;
    this.to = to;
  ...
  ReplaceAroundStep2.prototype.map = function map13(mapping) {
    var from3 = mapping.mapResult(this.from, 1), to = mapping.mapResult(this.to, -1);
    var gapFrom = mapping.map(this.gapFrom, -1), gapTo = mapping.map(this.gapTo, 1);
    if (from3.deleted && to.deleted || gapFrom < from3.pos || gapTo > to.pos) {
      return null;
    }
    return new ReplaceAroundStep2(from3.pos, to.pos, gapFrom, gapTo, this.slice, this.insert, this.structure);
  };
...
function contentBetween(doc2, from3, to) {
  var $from = doc2.resolve(from3), dist = to - from3, depth = $from.depth;
...
Transform.prototype.setBlockType = function(from3, to, type, attrs) {
  var this$1 = this;
  if (to === void 0)
    to = from3;
  if (!type.isTextblock) {
  ...
  this.doc.nodesBetween(from3, to, function(node4, pos) {
...
function dropPoint(doc2, pos, slice3) {
  ...
  var content2 = slice3.content;
  for (var i = 0; i < slice3.openStart; i++) {
    content2 = content2.firstChild.content;
  }
  for (var pass = 1; pass <= (slice3.openStart == 0 && slice3.size ? 2 : 1); pass++) {
...
var AddMarkStep = function(Step3) {
  function AddMarkStep2(from3, to, mark3) {
    Step3.call(this);
    this.from = from3;
    this.to = to;
    this.mark = mark3;
  ...
  AddMarkStep2.prototype.apply = function apply8(doc2) {
    ...
    var slice3 = new Slice(mapFragment(oldSlice.content, function(node4, parent2) {
  ...
  AddMarkStep2.prototype.map = function map13(mapping) {
    var from3 = mapping.mapResult(this.from, 1), to = mapping.mapResult(this.to, -1);
    if (from3.deleted && to.deleted || from3.pos >= to.pos) {
      return null;
    }
    return new AddMarkStep2(from3.pos, to.pos, this.mark);
  };
...
var RemoveMarkStep = function(Step3) {
  function RemoveMarkStep2(from3, to, mark3) {
    Step3.call(this);
    this.from = from3;
    this.to = to;
    this.mark = mark3;
  ...
  RemoveMarkStep2.prototype.apply = function apply8(doc2) {
    ...
    var slice3 = new Slice(mapFragment(oldSlice.content, function(node4) {
  ...
  RemoveMarkStep2.prototype.map = function map13(mapping) {
    var from3 = mapping.mapResult(this.from, 1), to = mapping.mapResult(this.to, -1);
    if (from3.deleted && to.deleted || from3.pos >= to.pos) {
      return null;
    }
    return new RemoveMarkStep2(from3.pos, to.pos, this.mark);
  };
...
Transform.prototype.addMark = function(from3, to, mark3) {
  var this$1 = this;
  var removed = [], added = [], removing = null, adding = null;
  this.doc.nodesBetween(from3, to, function(node4, pos, parent) {
    if (!node4.isInline) {
      return;
    }
    var marks2 = node4.marks;
    if (!mark3.isInSet(marks2) && parent.type.allowsMarkType(mark3.type)) {
      var start3 = Math.max(pos, from3), end2 = Math.min(pos + node4.nodeSize, to);
...
Transform.prototype.removeMark = function(from3, to, mark3) {
  var this$1 = this;
  if (mark3 === void 0)
    mark3 = null;
  var matched = [], step2 = 0;
    ...
    if (toRemove && toRemove.length) {
      ...
      for (var i = 0; i < toRemove.length; i++) {
        ...
        if (found$1) {
          ...
          matched.push({ style: style2, from: Math.max(pos, from3), to: end2, step: step2 });
        }
      }
    }
  });
...
function replaceStep(doc2, from3, to, slice3) {
  if (to === void 0)
    to = from3;
  if (slice3 === void 0)
    slice3 = Slice.empty;
  ...
  var $from = doc2.resolve(from3), $to = doc2.resolve(to);
  if (fitsTrivially($from, $to, slice3)) {
    return new ReplaceStep(from3, to, slice3);
  }
  return new Fitter($from, $to, slice3).fit();
}
Transform.prototype.replace = function(from3, to, slice3) {
  if (to === void 0)
    to = from3;
  if (slice3 === void 0)
    slice3 = Slice.empty;
  var step2 = replaceStep(this.doc, from3, to, slice3);
  if (step2) {
    this.step(step2);
  }
  return this;
};
Transform.prototype.replaceWith = function(from3, to, content2) {
  return this.replace(from3, to, new Slice(Fragment.from(content2), 0, 0));
};
Transform.prototype.delete = function(from3, to) {
...
Fitter.prototype.fit = function fit() {
  ...
  var slice3 = new Slice(content2, openStart, openEnd);
  if (moveInline > -1) {
...
Fitter.prototype.placeNodes = function placeNodes(ref) {
  ...
  this.unplaced = !toEnd ? new Slice(dropFromFragment(slice3.content, sliceDepth, taken), slice3.openStart, slice3.openEnd) : sliceDepth == 0 ? Slice.empty : new Slice(
    dropFromFragment(slice3.content, sliceDepth - 1, 1),
    sliceDepth - 1,
    openEndCount < 0 ? slice3.openEnd : sliceDepth - 1
  );
};
...
Transform.prototype.replaceRange = function(from3, to, slice3) {
  if (!slice3.size) {
    return this.deleteRange(from3, to);
  }
  var $from = this.doc.resolve(from3), $to = this.doc.resolve(to);
  if (fitsTrivially($from, $to, slice3)) {
    return this.step(new ReplaceStep(from3, to, slice3));
  }
  ...
  for (var content2 = slice3.content, i = 0; ; i++) {
    ...
    for (var i$1 = 0; i$1 < targetDepths.length; i$1++) {
      ...
      if (parent.canReplaceWith(index2, index2, insert.type, insert.marks)) {
        return this.replace(
          ...
          new Slice(
            closeFragment(slice3.content, 0, slice3.openStart, openDepth),
            openDepth,
            slice3.openEnd
          )
        );
...
Transform.prototype.replaceRangeWith = function(from3, to, node4) {
  if (!node4.isInline && from3 == to && this.doc.resolve(from3).parent.content.size) {
    var point = insertPoint(this.doc, from3, node4.type);
    if (point != null) {
      from3 = to = point;
    }
  }
  return this.replaceRange(from3, to, new Slice(Fragment.from(node4), 0, 0));
};
Transform.prototype.deleteRange = function(from3, to) {
  var $from = this.doc.resolve(from3), $to = this.doc.resolve(to);
...
Selection.prototype.replaceWith = function replaceWith(tr, node4) {
  ...
  for (var i = 0; i < ranges.length; i++) {
    ...
    var $to = ref.$to;
    var mapping = tr.mapping.slice(mapFrom);
    var from3 = mapping.map($from.pos), to = mapping.map($to.pos);
    if (i) {
      tr.deleteRange(from3, to);
    } else {
      tr.replaceRangeWith(from3, to, node4);
...
var TextSelection = function(Selection3) {
  ...
  TextSelection2.prototype.map = function map13(doc2, mapping) {
...
var NodeSelection = function(Selection3) {
  ...
  NodeSelection2.prototype.map = function map13(doc2, mapping) {
  ...
  NodeSelection2.create = function create5(doc2, from3) {
    return new this(doc2.resolve(from3));
  };
...
var AllSelection = function(Selection3) {
  ...
  AllSelection2.prototype.map = function map13(doc2) {
    return new AllSelection2(doc2);
  };
...
function selectionToInsertionEnd(tr, startLen, bias) {
  ...
  var map13 = tr.mapping.maps[last], end2;
  map13.forEach(function(_from, _to, _newFrom, newTo) {
    if (end2 == null) {
      end2 = newTo;
    }
  });
...
var Transaction = function(Transform3) {
  ...
  Transaction2.prototype.replaceSelection = function replaceSelection(slice3) {
  ...
  Transaction2.prototype.insertText = function insertText(text2, from3, to) {
    if (to === void 0)
      to = from3;
    ...
    if (from3 == null) {
      ...
      if (!marks2) {
        var $from = this.doc.resolve(from3);
        marks2 = to == from3 ? $from.marks() : $from.marksAcross(this.doc.resolve(to));
      }
      this.replaceRangeWith(from3, to, schema.text(text2, marks2));
...
function setBlockType(nodeType2, attrs) {
  return function(state, dispatch2) {
    ...
    state.doc.nodesBetween(from3, to, function(node4, pos) {
      if (applicable) {
        return false;
      }
    ...
    if (dispatch2) {
      dispatch2(state.tr.setBlockType(from3, to, nodeType2, attrs).scrollIntoView());
    }
    return true;
  };
}
...
function liftOutOfList(state, dispatch2, range) {
  ...
  if (!parent.canReplace(
    ...
    indexBefore + 1,
    item.content.append(atEnd2 ? Fragment.empty : Fragment.from(list))
  )) {
    return false;
  }
  ...
  tr.step(new ReplaceAroundStep(
    ...
    end2 - 1,
    new Slice(
      (atStart2 ? Fragment.empty : Fragment.from(list.copy(Fragment.empty))).append(atEnd2 ? Fragment.empty : Fragment.from(list.copy(Fragment.empty))),
      atStart2 ? 0 : 1,
      atEnd2 ? 0 : 1
    ),
    atStart2 ? 0 : 1
...
function sinkListItem(itemType) {
  return function(state, dispatch2) {
    ...
    if (dispatch2) {
      ...
      dispatch2(state.tr.step(new ReplaceAroundStep(
        before2 - (nestedBefore ? 3 : 1),
        after2,
        before2,
        after2,
        slice3,
...
var textRange = function(node4, from3, to) {
  var range = reusedRange || (reusedRange = document.createRange());
  range.setEnd(node4, to == null ? node4.nodeValue.length : to);
  range.setStart(node4, from3 || 0);
  return range;
};
...
function coordsAtPos(view, pos, side) {
  ...
  if (node4.nodeType == 3) {
    if (supportEmptyRange && (BIDI.test(node4.nodeValue) || (side < 0 ? !offset2 : offset2 == node4.nodeValue.length))) {
      ...
      var from3 = offset2, to = offset2, takeSide = side < 0 ? 1 : -1;
      if (side < 0 && !offset2) {
        to++;
        takeSide = -1;
      ...
      return flattenV(singleRect(textRange(node4, from3, to), takeSide), takeSide < 0);
    }
  }
...
ViewDesc.prototype.parseRange = function parseRange(from3, to, base2) {
  if (base2 === void 0)
    base2 = 0;
  ...
  for (var offset2 = base2, i = 0; ; i++) {
    ...
    if (fromOffset == -1 && from3 <= end2) {
      ...
      if (from3 >= childBase && to <= end2 - child3.border && child3.node && child3.contentDOM && this.contentDOM.contains(child3.contentDOM)) {
        return child3.parseRange(from3, to, childBase);
      }
      from3 = offset2;
      for (var j = i; j > 0; j--) {
...
ViewDesc.prototype.setSelection = function setSelection(anchor, head, root, force) {
  var from3 = Math.min(anchor, head), to = Math.max(anchor, head);
  for (var i = 0, offset2 = 0; i < this.children.length; i++) {
    var child3 = this.children[i], end2 = offset2 + child3.size;
...
ViewDesc.prototype.markDirty = function markDirty(from3, to) {
  for (var offset2 = 0, i = 0; i < this.children.length; i++) {
    ...
    if (offset2 == end2 ? from3 <= end2 && to >= offset2 : from3 < end2 && to > offset2) {
      ...
      if (from3 >= startInside && to <= endInside) {
        ...
        if (from3 == startInside && to == endInside && (child3.contentLost || child3.dom.parentNode != this.contentDOM)) {
          child3.dirty = NODE_DIRTY;
        } else {
          child3.markDirty(from3 - startInside, to - startInside);
        }
        return;
      } else {
...
var MarkViewDesc = function(ViewDesc3) {
  ...
  MarkViewDesc2.prototype.markDirty = function markDirty2(from3, to) {
    ViewDesc3.prototype.markDirty.call(this, from3, to);
  ...
  MarkViewDesc2.prototype.slice = function slice3(from3, to, view) {
...
var NodeViewDesc = function(ViewDesc3) {
  ...
  NodeViewDesc2.prototype.localCompositionInfo = function localCompositionInfo(view, pos) {
    ...
    if (this.node.inlineContent) {
      ...
      var textPos = findTextInFragment(this.node.content, text2, from3 - pos, to - pos);
...
var TextViewDesc = function(NodeViewDesc2) {
  ...
  TextViewDesc2.prototype.slice = function slice3(from3, to, view) {
    var node4 = this.node.cut(from3, to), dom = document.createTextNode(node4.text);
    return new TextViewDesc2(this.parent, node4, this.outerDeco, this.innerDeco, dom, dom, view);
  };
  TextViewDesc2.prototype.markDirty = function markDirty2(from3, to) {
    NodeViewDesc2.prototype.markDirty.call(this, from3, to);
...
var TrailingHackViewDesc = function(ViewDesc3) {
...
function replaceNodes(nodes, from3, to, view, replacement) {
  ...
  for (var i = 0, off = 0; i < nodes.length; i++) {
    ...
    if (start3 >= to || end2 <= from3) {
      result2.push(child3);
    } else {
      if (start3 < from3) {
        result2.push(child3.slice(0, from3 - start3, view));
      }
      if (replacement) {
...
function selectionToDOM(view, force) {
  ...
  if (!force && view.mouseDown && view.mouseDown.allowDefault) {
    ...
    if (domSel.anchorNode && isEquivalentPosition(
      domSel.anchorNode,
      domSel.anchorOffset,
      curSel.anchorNode,
      curSel.anchorOffset
...
function parseBetween(view, from_, to_) {
  ...
  var $from = startDoc.resolve(from3);
  ...
  if (find2 && find2[0].pos != null) {
    ...
    sel = { anchor: anchor$1 + from3, head: head + from3 };
  }
...
function readDOMChange(view, from3, to, typeOver, addedNodes) {
  ...
  var $before = view.state.doc.resolve(from3);
  var shared = $before.sharedDepth(to);
  from3 = $before.before(shared + 1);
  to = view.state.doc.resolve(to).after(shared + 1);
  var sel = view.state.selection;
  var parse3 = parseBetween(view, from3, to);
...
function serializeForClipboard(view, slice3) {
  var context = [];
  var content2 = slice3.content;
  var openStart = slice3.openStart;
  var openEnd = slice3.openEnd;
  ...
  var text2 = view.someProp("clipboardTextSerializer", function(f) {
    return f(slice3);
  }) || slice3.content.textBetween(0, slice3.content.size, "\n\n");
  return { dom: wrap, text: text2 };
}
function parseFromClipboard(view, text2, html, plainText, $context) {
  var dom, inCode = $context.parent.type.spec.code, slice3;
  if (!html && !text2) {
    return null;
  }
  ...
  if (sliceData) {
    slice3 = addContext(closeSlice(slice3, +sliceData[1], +sliceData[2]), sliceData[3]);
  } else {
    slice3 = Slice.maxOpen(normalizeSiblings(slice3.content, $context), true);
    if (slice3.openStart || slice3.openEnd) {
      var openStart = 0, openEnd = 0;
      for (var node4 = slice3.content.firstChild; openStart < slice3.openStart && !node4.type.spec.isolating; openStart++, node4 = node4.firstChild) {
      }
      for (var node$1 = slice3.content.lastChild; openEnd < slice3.openEnd && !node$1.type.spec.isolating; openEnd++, node$1 = node$1.lastChild) {
      }
...
function withWrappers(node4, wrap, from3) {
  if (from3 === void 0)
    from3 = 0;
  for (var i = wrap.length - 1; i >= from3; i--) {
...
function addContext(slice3, context) {
  ...
  var schema = slice3.content.firstChild.type.schema, array;
  try {
...
DOMObserver.prototype.flush = function flush() {
  ...
  var from3 = -1, to = -1, typeOver = false, added = [];
  if (this.view.editable) {
    for (var i = 0; i < mutations.length; i++) {
      var result$1 = this.registerMutation(mutations[i], added);
      if (result$1) {
        from3 = from3 < 0 ? result$1.from : Math.min(result$1.from, from3);
  ...
  if (from3 > -1 || newSel) {
    if (from3 > -1) {
      this.view.docView.markDirty(from3, to);
      checkCSS(this.view);
    }
...
DOMObserver.prototype.registerMutation = function registerMutation(mut, added) {
  ...
  if (mut.type == "childList") {
    ...
    var from3 = desc.localPosFromDOM(mut.target, fromOffset, -1);
...
function initInput(view) {
  ...
  view.domObserver = new DOMObserver(view, function(from3, to, typeOver, added) {
    return readDOMChange(view, from3, to, typeOver, added);
  });
  view.domObserver.start();
...
MouseDown.prototype.up = function up(event) {
  ...
  if (this.allowDefault || !pos) {
    ...
    event.preventDefault();
  } else if (event.button == 0 && (this.flushed || result.safari && this.mightDrag && !this.mightDrag.node.isAtom || result.chrome && !(this.view.state.selection instanceof TextSelection) && Math.min(
...
handlers.copy = editHandlers.cut = function(view, e) {
  ...
  var slice3 = sel.content();
  var ref = serializeForClipboard(view, slice3);
  var dom = ref.dom;
  var text2 = ref.text;
  if (data) {
    e.preventDefault();
...
function doPaste(view, text2, html, e) {
  var slice3 = parseFromClipboard(view, text2, html, view.shiftKey, view.state.selection.$from);
  if (view.someProp("handlePaste", function(f) {
    return f(view, e, slice3 || Slice.empty);
  })) {
    return true;
  }
  if (!slice3) {
  ...
  var singleNode = sliceSingleNode(slice3);
  var tr = singleNode ? view.state.tr.replaceSelectionWith(singleNode, view.shiftKey) : view.state.tr.replaceSelection(slice3);
...
var Dragging = function Dragging2(slice3, move2) {
  this.slice = slice3;
  this.move = move2;
};
...
handlers.dragstart = function(view, e) {
  ...
  var slice3 = view.state.selection.content();
  var ref = serializeForClipboard(view, slice3);
  var dom = ref.dom;
  var text2 = ref.text;
  ...
  view.dragging = new Dragging(slice3, !e[dragCopyModifier]);
};
...
editHandlers.drop = function(view, e) {
  ...
  var slice3 = dragging && dragging.slice;
  if (slice3) {
    view.someProp("transformPasted", function(f) {
      slice3 = f(slice3);
    });
  } else {
    slice3 = parseFromClipboard(
      view,
  ...
  if (view.someProp("handleDrop", function(f) {
    return f(view, e, slice3 || Slice.empty, move2);
  })) {
    e.preventDefault();
    return;
  }
  ...
  e.preventDefault();
  var insertPos = slice3 ? dropPoint(view.state.doc, $mouse.pos, slice3) : $mouse.pos;
  if (insertPos == null) {
    insertPos = $mouse.pos;
  }
  var tr = view.state.tr;
  ...
  var isNode = slice3.openStart == 0 && slice3.openEnd == 0 && slice3.content.childCount == 1;
...
InlineType.prototype.map = function map8(mapping, span, offset2, oldOffset) {
  var from3 = mapping.map(span.from + oldOffset, this.spec.inclusiveStart ? -1 : 1) - offset2;
...
NodeType3.prototype.map = function map9(mapping, span, offset2, oldOffset) {
  var from3 = mapping.mapResult(span.from + oldOffset, 1);
  if (from3.deleted) {
    return null;
  }
...
var Decoration = function Decoration2(from3, to, type) {
  this.from = from3;
  this.to = to;
  this.type = type;
};
var prototypeAccessors$14 = { spec: { configurable: true }, inline: { configurable: true } };
Decoration.prototype.copy = function copy4(from3, to) {
...
Decoration.inline = function inline(from3, to, attrs, spec) {
  return new Decoration(from3, to, new InlineType(attrs, spec));
};
Decoration.node = function node3(from3, to, attrs, spec) {
  return new Decoration(from3, to, new NodeType3(attrs, spec));
};
...
DecorationSet.prototype.removeInner = function removeInner(decorations, offset2) {
  ...
  for (var i = 0; i < children.length; i += 3) {
    var found2 = void 0, from3 = children[i] + offset2, to = children[i + 1] + offset2;
    ...
    var removed = children[i + 2].removeInner(found2, from3 + 1);
    if (removed != empty) {
...
DecorationSet.prototype.forChild = function forChild(offset2, node4) {
  ...
  for (var i$1 = 0; i$1 < this.local.length; i$1++) {
    ...
    if (dec.from < end2 && dec.to > start3 && dec.type instanceof InlineType) {
      var from3 = Math.max(start3, dec.from) - start3, to = Math.min(end2, dec.to) - start3;
      if (from3 < to) {
...
function mapChildren(oldChildren, newLocal, mapping, node4, offset2, oldOffset, options) {
  ...
  for (var i$1 = 0; i$1 < children.length; i$1 += 3) {
    if (children[i$1 + 1] == -1) {
      var from3 = mapping.map(oldChildren[i$1] + oldOffset), fromLocal = from3 - offset2;
      ...
      if (childNode && childOffset == fromLocal && childOffset + childNode.nodeSize == toLocal) {
        var mapped = children[i$1 + 2].mapInner(mapping, childNode, from3 + 1, oldChildren[i$1] + oldOffset + 1, options);
        if (mapped != empty) {
...
function normalize(map13) {
  var copy5 = /* @__PURE__ */ Object.create(null);
  for (var prop in map13) {
    copy5[normalizeKeyName(prop)] = map13[prop];
  }
  return copy5;
}
...
function keydownHandler(bindings) {
  var map13 = normalize(bindings);
  return function(view, event) {
    var name = keyName(event), isChar = name.length == 1 && name != " ", baseName;
    var direct = map13[modifiers(name, event, !isChar)];
    ...
    if (isChar && (event.shiftKey || event.altKey || event.metaKey || name.charCodeAt(0) > 127) && (baseName = base[event.keyCode]) && baseName != name) {
      var fromCode = map13[modifiers(baseName, event, true)];
      if (fromCode && fromCode(view.state, view.dispatch, view)) {
        return true;
      }
    } else if (isChar && event.shiftKey) {
      var withShift = map13[modifiers(name, event, true)];
...
function getTextBetween(startNode, range, options) {
  ...
  let text2 = "";
  let separated = true;
  startNode.nodesBetween(from3, to, (node4, pos, parent, index2) => {
    var _a;
    ...
    if (textSerializer) {
      ...
      text2 += (_a = node4 === null || node4 === void 0 ? void 0 : node4.text) === null || _a === void 0 ? void 0 : _a.slice(Math.max(from3, pos) - pos, to - pos);
      separated = false;
...
function selectionToInsertionEnd2(tr, startLen, bias) {
  ...
  const map13 = tr.mapping.maps[last];
  let end2 = 0;
  map13.forEach((_from, _to, _newFrom, newTo) => {
    if (end2 === 0) {
      end2 = newTo;
    }
  });
...
var insertContentAt = (position, value, options) => ({ tr, dispatch: dispatch2, editor }) => {
  if (dispatch2) {
    ...
    tr.replaceWith(from3, to, content2);
...
function isNodeActive(state, typeOrName, attributes = {}) {
  ...
  const nodeRanges = [];
  state.doc.nodesBetween(from3, to, (node4, pos) => {
    if (node4.isText) {
      return;
    }
...
function getMarkAttributes(state, typeOrName) {
  ...
  if (empty2) {
    ...
    state.doc.nodesBetween(from3, to, (node4) => {
...
function getNodeAttributes(state, typeOrName) {
  ...
  const nodes = [];
  state.doc.nodesBetween(from3, to, (node5) => {
    nodes.push(node5);
  });
...
function run(config) {
  const { editor, state, from: from3, to, rules } = config;
  const { commands, chain, can } = new CommandManager({
    editor,
    state
  });
...
function pasteRulesPlugin(props) {
  ...
  const plugin = new Plugin({
    ...
    appendTransaction: (transactions, oldState, state) => {
      ...
      const from3 = before2.content.findDiffStart(doc2.content);
...
function getMarksBetween(from3, to, doc2) {
  const marks2 = [];
  if (from3 === to) {
    doc2.resolve(from3).marks().forEach((mark3) => {
    ...
    doc2.nodesBetween(from3, to, (node4, pos) => {
...
//# sourceMappingURL=chunk-UVT663EJ.js.map
